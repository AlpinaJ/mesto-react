...
function PopupWithForm({isOpen, name, closePopup, title, children, buttonType, buttonContent}){
    return(
        <div className={isOpen ? `popup popup_type_${name} popup_opened` : `popup popup_type_${name}`}>
            ...
            <div className="popup__container">
                ...
                <form className={`popup__form popup__form_type_${name}`} name={name}>
                    {children}
                    <button type="submit" className={`popup__button popup__button-${buttonType}`}> {buttonContent}</button>
                </form>
            </div>
        </div>
    )
